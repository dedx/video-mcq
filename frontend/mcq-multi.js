...
License:
  ...
  along with this project. If not, see <https://www.gnu.org/licenses/>.
   ...
   mcq-multi.js — identity submit is the final submit when requireIdentity=true.
   Keeps: quiz picker, gating, per-choice feedback/chips, watch coverage,
   one-time forward seek warning/bounce, idempotent submit protection.
*/
...
(function () {
  ...
  function ensureGroupAndQuizPickers() {
      const host = $('player');

      let groupPicker = $('groupPicker');
      if (!groupPicker) {
	  groupPicker = document.createElement('select');
	  groupPicker.id = 'groupPicker';
	  groupPicker.style.display = 'block';
	  groupPicker.style.margin = '8px 0';
	  if (host && host.parentNode) host.parentNode.insertBefore(groupPicker, host);
      }
      
      let quizPicker = $('quizPicker');
      if (!quizPicker) {
	  quizPicker = document.createElement('select');
	  quizPicker.id = 'quizPicker';
	  quizPicker.style.display = 'block';
	  quizPicker.style.margin = '8px 0';
	  if (host && host.parentNode) host.parentNode.insertBefore(quizPicker, host);
      }
      
      return { groupPicker, quizPicker };
  }
  ...
  function addChoiceRow({labelText,inputType,name,value,isCorrect,container}){
    ...
    const fb = document.createElement('div'); fb.style.fontSize='.9rem'; fb.style.opacity='.95'; fb.style.marginTop='6px';
  ...
  function bindTheaterToggle() {
    ...
    if (btn.dataset.bound === '1') {
      // keep the label correct even on re-loads
      btn.textContent = document.body.classList.contains('theater') ? 'Exit Theater' : 'Theater';
      return;
    }
    btn.dataset.bound = '1';
    btn.textContent = document.body.classList.contains('theater') ? 'Exit Theater' : 'Theater';
    btn.addEventListener('click', () => {
      ...
      btn.textContent = document.body.classList.contains('theater') ? 'Exit Theater' : 'Theater';
      // Let layout settle, then normalize iframe sizing
      requestAnimationFrame(() => requestAnimationFrame(normalizePlayerIframe));	
    });
  }

  function normalizePlayerIframe() {
    try {
  ...
  //If the video should end early, keep track of the effective watch endpoint  
  ...
  function updateProgress(now = 0, dur = 0) {
    ...
    if (p) p.textContent = `${fmt(pts)}/${fmt(max)} • (${fmt(pct)}%)`;
    const fin = $('finish');
    if (fin) {
      if (quiz.requireIdentity === true) fin.classList.add('hidden');
      else {
  ...
  async function submitAttemptNow(viewerText) {
    ...
    answered.set('identity', { kind:'fr', text: viewerText });
  ...
  async function handleFinish() {
    if (quiz && quiz.requireIdentity === true) return; // identity path handles submit
    if (submitting || submittedOnce) return;
    submitting = true;
    const finBtn = $('finish');
    ...
    for (let i = quiz.items.length - 1; i >= 0; i--) {
      ...
      if (t === 'fr' || t === 'free' || t === 'free_response') {
        const a = answered.get(it.id);
        if (a && a.text) { viewer = a.text.trim(); break; }
      }
    }
    ...
    try {
      const r = await fetch(`/api/attempt/${encodeURIComponent(quiz.id || 'unknown')}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify(payload)
  ...
  // ======================================================================                           
  // Boot & quiz loading                                                                              
  // Initialize player, quiz picker, and load quizzes.                                                
  // ======================================================================                           
  /* Initialize player, quiz picker, and load quizzes. */
    
  // ---------- boot ----------
  async function boot() {
    const { groupPicker, quizPicker } = ensureGroupAndQuizPickers();
    bindTheaterToggle();
...
//	  a.localeCompare(b)
//    );
    const groupNames = ['Week 01', 'Week 02', 'Week 03', 'Week 04', 'Week 05',                      
                        'Week 06', 'Week 07', 'Week 08', 'Week 09', 'Week 10', 'Other']
    ...
    // ---- populate group picker ----
	  ...
	  o.value = g;
	  o.textContent = g;
	  groupPicker.appendChild(o);
      }
      
      // ---- helper to fill quiz picker ----
      function fillQuizPicker(groupName) {
	  ...
	  for (const q of arr) {
	      ...
	      quizPicker.appendChild(o);
	  }
	  
	  if (arr.length) loadQuiz(arr[0].id);
      }
      
      // ---- wire events ----
      groupPicker.addEventListener('change', () =>
	  fillQuizPicker(groupPicker.value)
      );
      
      quizPicker.addEventListener('change', () =>
	  loadQuiz(quizPicker.value)
      );
      
      // ---- initialize ----
/*      if (groupNames.length) {
	  fillQuizPicker(groupNames[0]);
      } else if (window.QUIZ_ID) {
	  loadQuiz(window.QUIZ_ID);
      } else {
	  const s = $('status');
	  if (s) s.textContent = 'No quizzes available.';
      }
*/
//    picker.addEventListener('change', () => loadQuiz(picker.value));
//    await loadQuiz(list[0].id);
      if (window.QUIZ_ID && list.some(q => q.id === window.QUIZ_ID)) {
	  // Find the group containing the quiz
	  const q = list.find(q => q.id === window.QUIZ_ID);
	  const g = q.group || 'Other';
	  
	  groupPicker.value = g;
	  fillQuizPicker(g);
	  
	  quizPicker.value = window.QUIZ_ID;
	  loadQuiz(window.QUIZ_ID);
  ...
  function resetUIForNewQuiz() {
    // Hide any overlay and clear its content
    const ov = $('overlay'); if (ov) ov.classList.add('hidden');
    overlayOpen = false; currentItem = null;

    const s = $('status');   if (s) s.textContent = '';
    const pr = $('prompt');  if (pr) pr.textContent = '';
    const fb = $('feedback');if (fb) fb.textContent = '';
    const box = $('choices');if (box) box.innerHTML = '';

    // Put buttons back to their default state
    ...
    if (contBtn) {
      ...
      contBtn.onclick = handleContinue; // restore default action
    }
    const finBtn = $('finish');
    if (finBtn) {
      finBtn.disabled = false;
      finBtn.textContent = 'Finish & Submit';
      finBtn.classList.add('hidden'); // updateProgress will toggle later if needed
    }
  }
  
  async function loadQuiz(qid) {
    // reset per-quiz state
    quiz = null; answered = new Map(); reviewedThisPass = new Set();
    overlayOpen = false; currentItem = null; watchedToEnd = false;
    lastNow = 0; watching = false; segments.length = 0; warnedSeekOnce = false;
    reviewMode = false; peakTime = 0; reviewExitTime = 0;
    submittedOnce = false; submitting = false;
    ATTEMPT_NONCE = (Math.random().toString(36).slice(2) + '-' + Date.now());
    // reset any leftover UI from the previous quiz
    resetUIForNewQuiz();
    // stop any previous timer until the new player is ready
        ...
        onStateChange: (e) => {
          if (e.data === YT.PlayerState.ENDED) {
            try {
              ...
              // close the active segment right at the real end
              stopWatch(now);

              // only mark watchedToEnd if coverage is essentially complete
              const effEnd = effectiveEnd(dur);
              const sec    = watchedSeconds(now, effEnd);
              const pct    = effEnd ? (sec / effEnd) * 100 : 0;
              if ((effEnd - sec) <= 1.0 || pct >= 97) {
                watchedToEnd = true;
              }
              updateProgress(now, dur);
            } catch {}

            // Idempotent identity open (guard against double-open / post-submit)
            if (quiz.requireIdentity &&
                !answered.has(ID_KEY) &&
                !overlayOpen && !submitting && !submittedOnce) {
              renderIdentity();
            } else {
              updateProgress();
            }
          ...
          if (e.data === YT.PlayerState.PAUSED || e.data === YT.PlayerState.BUFFERING) {
            try { const now = player.getCurrentTime() || 0; stopWatch(now); } catch {}
          }
        }
      }
    });
  ...
  loadYouTubeAPIThen(() => { (async () => { await boot(); })().catch(console.error); });
  ...
  function tick() {
    ...
    // Enforce endAt: stop the video at the effective end, even if YouTube's
    // natural duration is longer (i.e., trimmed ends).
    {
      const endCap = effectiveEnd(dur);
      if (endCap > 0 && now >= endCap - 0.05) {
      // close the active coverage segment at the true playhead
      stopWatch(now);

      // hold the player at the trimmed end and pause
      try {
        player.pauseVideo();
        player.seekTo(endCap, true);
      } catch {}

      // optional: mark finished if coverage is "close enough"
      // (percent remains coverage-only; this flag no longer forces 100%)
      const sec = watchedSeconds(now, endCap);
      if ((endCap - sec) <= 1.0 || (sec / endCap) >= 0.97) {
        watchedToEnd = true;
      }

      updateProgress(now, dur);

      // If identity is required, open it idempotently here too.
      if (quiz.requireIdentity &&
          !answered.has(ID_KEY) &&
          !overlayOpen && !submitting && !submittedOnce) {
        renderIdentity();
        return; // don't fall through this tick
      }
      return; // we handled the trimmed end
    }
  }
